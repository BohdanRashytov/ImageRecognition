...
object PetuninFirstRank {
  ...
  private def conductExtraExperiment(experiment: Experiment): Experiment = {
    if (experiment.samples.size < 1425) return experiment.copy(diagnosis = Some(Indeterminately))
    ...
    if (diagnosis == Indeterminately) {
      val deleteCellCancer = getCancerInfo._4.map(p => Math.abs((p.x - getCancerInfo._1.center.x) * (p.x - getCancerInfo._1.center.x) +
        (p.y - getCancerInfo._1.center.y) * (p.y - getCancerInfo._1.center.y) - getCancerInfo._1.R))
      val indexCancer = deleteCellCancer.indexOf(deleteCellCancer.min)

      val deleteCellFibroadenomatosis = getFibroadenomatosisInfo._4.map(p => Math.abs((p.x - getFibroadenomatosisInfo._1.center.x) * (p.x - getFibroadenomatosisInfo._1.center.x) +
        (p.y - getFibroadenomatosisInfo._1.center.y) * (p.y - getFibroadenomatosisInfo._1.center.y) - getFibroadenomatosisInfo._1.R))
      val indexFibroadenomatosis = deleteCellFibroadenomatosis.indexOf(deleteCellFibroadenomatosis.min)

      conductExtraExperiment(experiment.copy(samples =
        experiment.samples.filter(_.diagnosis == Cancer).filter(c => c == experiment.samples.filter(_.diagnosis == Cancer)(indexCancer)) :::
          experiment.samples.filter(_.diagnosis == Fibroadenomatosis).filter(c => c == experiment.samples.filter(_.diagnosis == Fibroadenomatosis)(indexFibroadenomatosis))))}
