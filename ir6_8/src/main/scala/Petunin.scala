...
object Petunin {
  ...
  private def conductExtraExperiment(experiment: Experiment): Experiment = {
    ...
    val pointsCancer = experiment.samples.filterNot(_.diagnosis == Cancer).map(c => Point(c.observations(0), c.observations(1)))
    val pointsFibroadenomatosis = experiment.samples.filterNot(_.diagnosis == Fibroadenomatosis).map(c => Point(c.observations(0), c.observations(1)))
    ...
    if (!cancerIndicator && fibroadenomatosisIndicator) Fibroadenomatosis
  ...
  private def calculate(p: List[Point]) = {
    ...
    (Ellipse(center, R, 1 / coef), angle, transferPoint)
  }
}
